...
Em **Storage**, crie um bucket chamado exatamente:
...
As policies de escrita já estão em `supabase.sql`: somente a administradora pode enviar/alterar/remover arquivos na pasta dela.
...
6. cadastrar produto sem foto;
7. cadastrar produto tirando foto pelo celular;
8. clicar em **Chegou 1** e confirmar entrada no histórico;
9. clicar em **Vendi 1** e confirmar venda no histórico;
10. tentar vender quando o estoque estiver zerado;
11. confirmar que estoque 0 não aparece no catálogo;
12. ocultar um produto com estoque e confirmar que ele some do catálogo;
13. editar preço/nome/foto e confirmar que o estoque não é alterado;
14. abrir o botão do WhatsApp e conferir número e mensagem.
...
4. nos acessos seguintes, o código de 6 dígitos é solicitado;
5. somente uma sessão `aal2` consegue ler/escrever dados administrativos no banco e no Storage.
...
**Recuperação:** o Supabase Auth não fornece recovery codes para TOTP. Para uma operação real, considere cadastrar um segundo fator de backup em dispositivo/local seguro ou documentar o reset administrativo do fator pelo responsável técnico.
...
O histórico registra cada entrada e venda. A quantidade não pode ser alterada diretamente pela aplicação: o ajuste passa pela função atômica `adjust_stock`.
